...
def krylovsolve(
    ...
    * "store_final_state": store final state even though expectation values are
    ...
    if sparse:
        _H = _data.to('csr', H.data).as_scipy()  # (fast_) csr_matrix
    else:
    ...
    if T_m.shape[0] < krylov_dim + 1:
        ...
        if n_timesteps >= options['nsteps']:
            raise Exception(
                ...
                "be increased via the 'SolverOptions.nsteps' property."
            )
...
def _evolve(t0: float, krylov_basis: np.ndarray, T_m: np.ndarray):
    ...
    eigenvalues, eigenvectors = eigs(_data.Dense(T_m), True)
...
def _lanczos_error_equation_to_optimize_delta_t(
    ...
    Lanczos iteractions.
    """
    eigenvalues1, eigenvectors = eigs(_data.Dense(T[0:, 0:]), True)
    eigenvectors1 = eigenvectors.to_array()
    U1 = np.matmul(krylov_basis[0:, 0:].T, eigenvectors1)
    e01 = eigenvectors1.conj().T[:, 0]

    eigenvalues2, eigenvectors = eigs(_data.Dense(T[0:-1, 0: T.shape[1] - 1]), True)
    eigenvectors2 = eigenvectors.to_array()
