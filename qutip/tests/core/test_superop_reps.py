...
from qutip import (
    ...
    kraus_to_super, to_super, to_superpauli, from_superpauli, to_choi,
...
class TestSuperopReps:
    ...
    def test_SuperPauliRoundTrip(self):
        ...
        is an identity: from_superpauli(to_superpauli(S)) == S.
        """
        superop = rand_super(2)

        pauli_rep = to_superpauli(superop)
        back_to_super = from_superpauli(pauli_rep)

        assert (back_to_super - superop).norm() < 1e-12

    def test_SuperPauliInvalidDims(self):
        """
        Superoperator: Ensure to_superpauli and from_superpauli
        raises ValueError for invalid dims
        """
        qutrit_super = rand_super(3)
        non_square = np.ones((3, 4))
        invalid_dims = Qobj(np.eye(9))
        ...
        with pytest.raises(ValueError, match="must be a square."):
            from_superpauli(non_square)
        with pytest.raises(ValueError, match="defined for qubit systems."):
            from_superpauli(invalid_dims)
