...
def test_spin_q_function_normalized(spin, pure):
    ...
    norm = d / (4 * np.pi) * np.trapezoid(
        np.trapezoid(Q * np.sin(THETA), theta), phi
    )
...
def test_spin_wigner_normalized(spin, pure):
    ...
    norm = np.trapezoid(
        np.trapezoid(W * np.sin(THETA) * np.sqrt(d / (4*np.pi)), theta), phi
    )
...
def test_spin_wigner_overlap(spin, pure, n=5):
    ...
    for k in range(n):
        ...
        W_overlap = np.trapezoid(
            np.trapezoid(W_state * W * np.sin(THETA), theta), phi).real
