...
try:
    import qutip.solver.heom.backend_petsc  # Register PETSc backend
except ImportError:
    pass
...
class TestPETScBackend:
    def test_pure_dephasing_model(self, atol=1e-2):
        lam = 0.025
        gamma = 0.05
        T = 1 / 0.95
        Nk = 2
        ...
        options_petsc = {"method": "petsc", "ts_type": "bdf", "atol": 1e-8, "rtol": 1e-8}
    ...
    def test_steady_state_fidelity(self, atol=1e-2):
    ...
    def test_steady_state_vs_csr(self, atol=1e-2):
        H_sys = 0.25 * sigmaz() + 0.5 * sigmax()
        bath = DrudeLorentzBath(sigmaz(), lam=0.025, gamma=0.05, T=1 / 0.95, Nk=2)
        solver_petsc = HEOMSolver(H_sys, bath, 5, options={"method": "petsc"})
        solver_csr = HEOMSolver(H_sys, bath, 5)
        rho_ss_petsc, ados_petsc = solver_petsc.steady_state(ksp_type="gmres", pc_type="lu", ksp_rtol=1e-6, ksp_atol=1e-6)
    ...
    def test_steady_state_trace_and_hermiticity(self, atol=1e-6):
        ...
        rho_ss, _ = solver.steady_state(ksp_type="gmres", pc_type="lu", ksp_rtol=1e-6, ksp_atol=1e-6)
    ...
    def test_steady_state_is_fixed_point(self, atol=1e-2):
        H_sys = 0.25 * sigmaz() + 0.5 * sigmax()
        bath = DrudeLorentzBath(sigmaz(), lam=0.025, gamma=0.05, T=1 / 0.95, Nk=2)
        solver = HEOMSolver(H_sys, bath, 5, options={"method": "petsc", "ts_type": "bdf", "atol": 1e-8, "rtol": 1e-8})
        rho_ss, steady_ados = solver.steady_state(ksp_type="gmres", pc_type="lu", ksp_rtol=1e-6, ksp_atol=1e-6)
    ...
    def test_steady_state_ksp_options(self, atol=1e-2):
        H_sys = sigmax()
        bath = DrudeLorentzBath(sigmaz(), lam=0.05, gamma=0.1, T=1.0, Nk=2)
        solver = HEOMSolver(H_sys, bath, 4, options={"method": "petsc"})
        rho_ss_default, _ = solver.steady_state(ksp_type="gmres", pc_type="lu", ksp_rtol=1e-6, ksp_atol=1e-6)
