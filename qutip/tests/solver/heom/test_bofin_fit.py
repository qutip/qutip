...
class TestSpectralFitter:
    ...
    def test_get_fit(self):
        ...
        assert np.isclose(bath.spectral_density_approx(w), ud.spectral_density(w)).all()
...
class TestCorrelationFitter:
    ...
    def test_corr_approx(self):
        t = np.linspace(0,10,100) #TODO FIXX TESTS
        C = np.exp(-t)
        bath = CorrelationFitter(sigmax(), 1,t,C)
        t2 = np.linspace(0,20,100) #TODO FIXX TESTS
        C2=np.exp(-t2)
        assert np.isclose(C2, bath._C_fun(t2)).all()
