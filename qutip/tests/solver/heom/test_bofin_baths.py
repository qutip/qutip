...
class TestBosonicBath:
    def test_create(self):
        Q = sigmaz()
        bath = BosonicBath(Q, [1.0], [0.5], [2.0], [0.6])
...
class TestFermionicBath:
    def test_create(self):
        Q = 1j * sigmaz()
        bath = FermionicBath(Q, [1.0], [0.5], [2.0], [0.6])
        ...
        bath = FermionicBath(Q, [1.0], [0.5], [2.0], [0.6], tag="bath1")
        exp_p, exp_m = bath.exponents
        check_exponent(
            exp_p,
            "+",
        ...
        with pytest.raises(ValueError) as err:
            FermionicBath("not-a-qobj", [1.0], [0.5], [2.0], [0.6])
        assert str(err.value) == "The coupling operator Q must be a Qobj."

        with pytest.raises(ValueError) as err:
            FermionicBath(Q, [1.0], [], [2.0], [0.6])
        assert str(err.value) == (
