...
class TestLindbladMatrixFormProperties:
    """Property tests for LindbladMatrixForm correctness with different operator types."""
    ...
    @pytest.mark.parametrize("case", [
        ...
        pytest.param({
            'N': 8,
            'H': [qutip.num(8), [qutip.create(8) + qutip.destroy(8), 'cos(t)']],
    ...
    def test_lindblad_matrix_form_vs_superoperator(self, case):
        """
        Compare LindbladMatrixForm against time-dependent Liouvillian superoperator.
        """
        N = case['N']
        H = case['H']
        c_ops = case['c_ops']
        ...
        # LindbladMatrixForm
        rhs_matrix = LindbladMatrixForm(QobjEvo(H), [QobjEvo(c) for c in c_ops])
