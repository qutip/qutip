...
def test_rand_herm(dimensions, density, distribution, dtype, random_generator):
    """
...
@pytest.mark.repeat(3)
@pytest.mark.parametrize('density', [0.2, 0.8], ids=["sparse", "dense"])
def test_rand_herm_Eigs(dimensions, density, random_generator):
    """
    ...
    eigs = random_generator.random(N)
    eigs /= np.sum(eigs)
    eigs.sort()
...
def test_rand_unitary(
    dimensions, distribution, density, dtype, random_generator
):
    """
    ...
    random_qobj = rand_unitary(
        ...
        density=density, dtype=dtype, seed=random_generator
    )
    I = qeye(dimensions)
...
def test_rand_dm(dimensions, kw, dtype, distribution, random_generator):
    """
...
def test_rand_stochastic(dimensions, kind, dtype, random_generator):
    ...
    random_qobj = rand_stochastic(
        dimensions, kind=kind, dtype=dtype, seed=random_generator
    )
...
def test_rand_ket(dimensions, distribution, dtype, random_generator):
    """
...
def test_rand_super(dimensions, dtype, superrep, random_generator):
    """
    Random Qobjs: Super operator.
    """
    random_qobj = rand_super(
        dimensions, dtype=dtype, superrep=superrep, seed=random_generator
    )
...
@pytest.mark.parametrize('superrep', ["choi", "super"])
def test_rand_super_bcsz(dimensions, dtype, rank, superrep, random_generator):
    """
...
def test_kraus_map(dimensions, dtype, random_generator):
    if isinstance(dimensions, list) and isinstance(dimensions[0], list):
        # Each element of a kraus map cannot be a super operators
        with pytest.raises(TypeError) as err:
            kmap = rand_kraus_map(
                dimensions, dtype=dtype, seed=random_generator
            )
        assert "super operator" in str(err.value)
    else:
        kmap = rand_kraus_map(dimensions, dtype=dtype, seed=random_generator)
...
def test_random_dtype(func, alias, dtype, random_generator):
    with CoreOptions(default_dtype=alias):
        object = func(2, seed=random_generator)
