...
def test_MCNoCollStrExpt():
    ...
    c_op_list = []
    tlist = linspace(0, 10, 100)
    mcdata = mcsolve(H, psi0, tlist, c_op_list, [a.dag(
    ) * a], args={'c': 0.0}, options=Odeoptions(gui=False))
    expt = mcdata.expect[0]
    actual_answer = 9.0 * ones(len(tlist))
    diff = mean(abs(actual_answer - expt) / actual_answer)
    assert_equal(diff < error, True)
...
def test_mc_dtypes1():
    ...
    data=mcsolve(H,psi0,tlist,[C1,C2],[C1dC1,C2dC2,a],ntraj=5,options=opts)
