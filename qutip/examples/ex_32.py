#
# This is a Monte-Carlo simulation showing the decay of a cavity Fock state |1>
# in a thermal environment with an average occupation number of n=0.063. Here,
# the coupling strength is given by the inverse of the cavity ring-down time
# Tc=0.129.
#
...
def run():
    ...
    # get expectation values from mc data (need extra index since ntraj is
    # list)
    ...
    fexpt = expect(a.dag() * a, final_state)  # find expectation value for
                                              # particle number
    ...
    ylim([-0.1, 1.5])
    ylabel('$\left< N \\right>$', fontsize=14)
    legend(('15 trajectories', 'master equation', 'steady state'),
           prop=leg_prop)
    ...
    ylim([-0.1, 1.5])
    ylabel('$\left< N \\right>$', fontsize=14)
    legend(('904 trajectories', 'master equation', 'steady state'),
           prop=leg_prop)
