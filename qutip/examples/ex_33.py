#
# Demonstrate the deviation from a thermal distribution for the trilinear
# Hamiltonian.
#
...
def run():
    ...
    x = arange(N1)
    params = {'axes.labelsize': 14, 'text.fontsize': 14,
    ...
    ax = Axes3D(fig)
    for j in range(5):
        ax.bar(x, diags1[10 * j], zs=tlist[10 * j], zdir='y',
               color=colors[j], linewidth=1.0, alpha=0.6, align='center')
        ax.plot(x, thermal[10 * j], zs=tlist[10 * j], zdir='y',
