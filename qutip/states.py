...
def thermal_dm(N, n, method = 'operator'):
    ...
    method : string {'operator', 'analytic'}
        ...
        thermal state probabilities. 'operator' (default) generates
        ...
        an infinite Hilbert space. ANALYTIC FORM IS NOT NORMALIZED. 
    ...
    if n==0:
        ...
        if method=='operator':
            ...
            rm = sp.spdiags(diags,0,N,N,format='csr') #populates diagonal terms using truncated operator expression
        elif method=='analytic':  
            rm = sp.spdiags((1.0+n)**(-1.0)*(n/(1.0+n))**(i),0,N,N,format='csr') #populates diagonal terms using analytic values
        else:
            raise ValueError("'method' keyword argument must be 'operator' or 'analytic'")
    return Qobj(rm)
