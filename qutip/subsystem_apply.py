...
def subsystem_apply(state, channel, mask, reference=False):
    ...
    #the same dimensions:
    aff_subs_dim_ar=[np.transpose(array(state.dims))[j] for j in range(len(\
    np.transpose(array(state.dims)))) if mask[j]]
    
    assert np.all([aff_subs_dim_ar[j]==aff_subs_dim_ar[0]\
    for j in range(len(aff_subs_dim_ar))]), \
    "Affected subsystems must have the same dimension. Given dimensions:"+\
    ...
    assert np.all(channel.shape==required_shape), "Superoperator dimension must \
    be the square of the subsystem dimension, given: "+repr(channel.shape)
...
def _top_apply_S(block,channel):    
    ...
    column=_block_col(block,*map(sqrt,channel.shape)) #FIXME use state shape?
    ...
    for row_idx in range(len(chan_mat)):
        ...
        temp_col[row_idx]=reduce(add,[scal[0,0]*mat for (scal,mat) in zip(transpose(row),column)])
...
def _block_col(mat_in,n_v,n_h):
    ...
    return np.reshape(array(_block_split(mat_in,n_v,n_h)).transpose(1,0,2,3),(n_v*n_h,rows/n_v,cols/n_h))
...
def _subsystem_apply_reference(state,channel,mask):
    ...
    if isoper(channel):
        ...
        kraus_list=[np.sqrt(vals[j])*vec2mat(vecs[j]) for j in range(len(vals))]
...
if __name__=='__main__':
    ...
    print "------------------------------------------------------"
    state=tensor([coherent_dm(2,0.2),coherent_dm(3,0.3),coherent_dm(5,0.5),\
    coherent_dm(3,0.5),coherent_dm(7,0.7j)])
    op_1=create(3)+destroy(3)
    analytic_output=tensor([coherent_dm(2,0.2),op_1*coherent_dm(3,0.3)*op_1\
    .dag(),coherent_dm(5,0.5),op_1*coherent_dm(3,0.5)*op_1.dag(),\
    coherent_dm(7,0.7j)])
    print "Difference between analytic result and Kraus map: \n"
    test_output=subsystem_apply(state,op_1,[False,True,False,True,False],\
    reference=True)
    ...
    efficient_output=subsystem_apply(state,op_1,[False,True,False,True,False])    
    ...
    state=tensor([coherent_dm(2,0.2),coherent_dm(3,0.3),coherent_dm(5,0.5),\
    coherent_dm(3,0.5),coherent_dm(7,0.7j)])
    superop_1 = propagator(Qobj(np.zeros([3,3])),0.2,[destroy(3),create(3),jmat(1.,'z')])
    inpt_5=vec2mat(superop_1.data.todense()*mat2vec(coherent_dm(3,0.5).data.todense()))    
    inpt_3=vec2mat(superop_1.data.todense()*mat2vec(coherent_dm(3,0.3).data.todense()))    
    dm_out_5=Qobj(inpt=inpt_5, type='oper')
    dm_out_3=Qobj(inpt=inpt_3, type='oper')
    analytic_output=tensor([coherent_dm(2,0.2),dm_out_3,coherent_dm(5,0.5),\
    dm_out_5,coherent_dm(7,0.7j)])
    print "Difference between analytic result and Kraus map: \n"
    test_output=subsystem_apply(state,superop_1,[False,True,False,True,False],\
    reference=True)
    print norm(test_output.data.todense()-analytic_output.data.todense())
    print "Difference between analytic result and efficient numerics: \n"
    efficient_output=subsystem_apply(state,superop_1,[False,True,False,True,False])    
    print norm(efficient_output.data.todense()-analytic_output.data.todense())
    ...
    superop_1 = propagator(Qobj(np.zeros([2,2])),0.2,[destroy(2),create(2),jmat(1/2.,'z')])
    inpt_2=vec2mat(superop_1.data.todense()*mat2vec(coherent_dm(2,0.2).data.todense()))    
    #inpt_3=vec2mat(superop_1.data.todense()*mat2vec(coherent_dm(3,0.3).data.todense()))    
    ...
    print("-----------------------------------------------------")    
    test_output=subsystem_apply(state,op_1,[False,True,False,True], reference=True)
    print "Difference between analytic result and Kraus map: \n"
    print norm(test_output.data.todense()-analytic_output.data.todense())        
