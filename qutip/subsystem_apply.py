...
#
# This code was contributed by Ben Criger. Resemblance to partial_transpose is
# intentional, and meant to enhance legibility.
#
#########################################################################
...
def subsystem_apply(state, channel, mask, reference=False):
    ...
    assert isket(state) or isoper(state),\
    "Input state must be a ket or oper, given: " + repr(state.type)

    assert issuper(channel) or isoper(channel),\
    "Input channel must be a super or oper, given: " + repr(channel.type)
...
def _subsystem_apply_ket(state, channel, mask):
    """
    Private placeholder function which applies channels to pure states,
    by calculating the rank-one projector associated with the input ket.
    """
...
def _block_apply(block, channel):
    """
    Private function; applies a channel directly to the first subsystem in a
    composite density matrix. Discerns whether the operator is represented
    on the original Hilbert space, or the associated Liouville space, then
    acts accordingly.
    """
    if isoper(channel):
...
def _top_apply_S(block, channel):
    """
    Uses scalar-matrix multiplication to efficiently apply a channel to the
    leftmost register in the tensor product, given a non-unitary propagator for
    a channel.
    """
    ...
    column = _block_col(block, *map(sqrt, channel.shape))
    ...
    for row_idx in range(len(chan_mat)):
        ...
        temp_col[row_idx] = reduce(add, [scal[0, 0] * mat
                                 for (scal, mat) in
                                 zip(transpose(row), column)])
...
if __name__ == '__main__':
    ...
    print "------------------------------------------------------"
    state = tensor([coherent_dm(2, 0.2), coherent_dm(3, 0.3),
                  coherent_dm(5, 0.5), coherent_dm(3, 0.5),
                  coherent_dm(7, 0.7j)])
    ...
    test_output = subsystem_apply(state, op_1,
    ...
    efficient_output = subsystem_apply(state, op_1,
                                     [False, True, False, True, False])
    print norm(
               efficient_output.data.todense() - analytic_output.data.todense()
              )
    ...
    state = tensor([coherent_dm(2, 0.2), coherent_dm(3, 0.3),
                  coherent_dm(5, 0.5), coherent_dm(3, 0.5),
                  coherent_dm(7, 0.7j)])
    superop_1 = propagator(
    Qobj(np.zeros([3, 3])), 0.2, [destroy(3), create(3), jmat(1., 'z')]
    )
    inpt_5 = vec2mat(superop_1.data.todense() *
    mat2vec(coherent_dm(3, 0.5).data.todense())
    )
    ...
    analytic_output = tensor([coherent_dm(2, 0.2), dm_out_3,
                            coherent_dm(5, 0.5), dm_out_5,
                            coherent_dm(7, 0.7j)])
    print "Difference between analytic result and Kraus map: \n"
    test_output = subsystem_apply(state, superop_1,
                                [False, True, False, True, False],
                                reference=True)
    print norm(test_output.data.todense() - analytic_output.data.todense())
    print "Difference between analytic result and efficient numerics: \n"
    efficient_output = subsystem_apply(state, superop_1,
                                     [False, True, False, True, False])
    print norm(
               efficient_output.data.todense() - analytic_output.data.todense()
              )
