...
def make_united_f_ptr_spline(ops, args, tlist, return_code=False, poly=False):
...
??? it would be easy to have different tlist for each array_like coefficients
    ...
    for op in ops:
        if op[3] == 2:
            ...
            spline, dt_cte = prep_cubic_spline(op[2], tlist, poly)
            ...
            if poly:
                if dt_cte:
                    if isinstance(op[2][0], (float, np.float32, np.float64)):
                        string = "spline_float_cte_poly(t, " + t_str + s_str +\
                                 N_times + ", " + dt_times + ")"
                        args_np += [[0,dt_cte]]
                    elif isinstance(op[2][0], (complex, np.complex128)):
                        string = "spline_complex_cte_poly(t, " + t_str + s_str +\
                                 N_times + ", " + dt_times + ")"
                        args_np += [[1,dt_cte]]
                else:
                    if isinstance(op[2][0], (float, np.float32, np.float64)):
                        string = "spline_float_t_poly(t, " + t_str + s_str +\
                                 N_times + ")"
                        args_np += [[0,dt_cte]]
                    elif isinstance(op[2][0], (complex, np.complex128)):
                        string = "spline_complex_t_poly(t, " + t_str + s_str +\
                                 N_times + ")"
                        args_np += [[1,dt_cte]]
            else:
                if dt_cte:
...
from qutip.cy.inter cimport spline_complex_t_poly, spline_complex_cte_poly
from qutip.cy.inter cimport spline_float_t_second, spline_float_cte_second
from qutip.cy.inter cimport spline_float_cte_poly, spline_float_t_poly
from qutip.cy.math cimport erf
cdef double pi = 3.14159265358979323
