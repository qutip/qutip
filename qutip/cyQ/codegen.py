...
class Codegen2():
    ...
    def func_header(self):
        ...
        for k in range(self.n_L_terms):
            arg_list.append("np.ndarray[CTYPE_t, ndim=1] data" + str(k) +
                            ", np.ndarray[int, ndim=1] idx" + str(k) +
                            ", np.ndarray[int, ndim=1] ptr" + str(k))
    ...
    def time_vars(self):
        ...
        for n in range(len(self.L_coeffs)):
            ...
            for m in ind:
                text = text.replace(
                    self.func_list[m], 'np.' + self.func_list[m])
...
def cython_spmv():
    ...
    """
    line0 = ("def spmv(np.ndarray[CTYPE_t, ndim=1] data, np.ndarray[int] " +
             "idx,np.ndarray[int] ptr,np.ndarray[CTYPE_t, ndim=1] vec):")
