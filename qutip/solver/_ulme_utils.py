...
def _make_L_eigs(H: qt.Qobj, X: qt.Qobj, g: callable):
    ...
    L_H = qt.data.multiply(X_H, qt.data.Dense(g(-np.subtract.outer(vals, vals))))
    L = vecs @ qt.Qobj(L_H, dims=X.dims) @ vecs.dag()
    return L
...
def _make_lambda_eigen(H, X, g):
    ...
    vecs = qt.Qobj(vecs)
    print(vecs)
    X_diag = qt.Qobj(vecs.dag() @ qt.Qobj(X @ vecs)).full()
    N = len(vals)
    fs = np.zeros((N, N, N), dtype=float)
    for i, j, k in itertools.product(range(N), repeat=3):
        fs[i, j, k] = f(vals[i] - vals[j], vals[j] - vals[k])
    LL = np.einsum("ijk,ij,jk->ik", fs, X_diag, X_diag)
    return vecs @ qt.Qobj(qt.Qobj(LL, dims=H.dims) @ vecs.dag())
...
def _make_lambda_prop(H: qt.QobjEvo, X: qt.QobjEvo, g: callable, T, Nt):
    ...
    if isinstance(X, qt.QobjEvo):
        def op(s, sp, t):
            if s == sp:
                return 0
            return U(t, s + t) @ X(s + t) @ U(s + t, sp + t) @ X(sp + t) @ U(sp + t, t) * (g(s) * g(-sp) * np.sign(s - sp) )
    else:
        def op(s, sp, t):
            if s == sp:
                return 0
            return U(t+ s, t ) * X * U(sp + t, s + t) * X * U(t, t + sp) * (g(s) * g(-sp) * np.sign(s - sp) )

    if isinstance(H,  qt.QobjEvo) or isinstance(X, qt.QobjEvo):
        return lambda t: integrate_2d_flat(op, t, T, Nt) * -0.5j

    return integrate_2d_flat(op, 0, T, Nt) * -0.5j
