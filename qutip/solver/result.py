...
class BaseResult:
    ...
    e_ops : :obj:`~Qobj`, :obj:`~QobjEvo`, function or list or dict of these
        ...
        object. ``.expect`` is a dictionary, whose keys are either the
        keys of ``e_ops`` if ``e_ops`` is a dict, the indexes of ``e_ops``
        if ``e_ops`` was a list, or the single key ``0`` if ``e_ops`` was
        a single item.
    ...
    expect : dict of expectation values
        The dictionary of expectation values. Each key is the key from the
        supplied ``e_ops`` or a list index if ``e_ops`` was a list or the
        single key ``0`` if ``e_ops`` was a single value.

        The value of each dict item is a list recording that expectation value
    ...
    e_ops : dict
        A dictionary containing the supplied e_ops.
