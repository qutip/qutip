   ...
   c_ops_and_rates : list of :class:`qutip.Qobj`.
    ...
    def __init__(
        ...
        *,
        ...
        if Nt != None:
            ...
            if taulist is not None:
                if taulist[1] - taulist[0] >= self.floquet_basis.T:
                    ...
                    taulist_zeroed = taulist - taulist[0]
                    Nt_finder = abs(taulist_zeroed + dt - self.floquet_basis.T)
        ...
        Rate_Qobj_list = [Qobj(
            RateMat,dims = [[self.Hdim,self.Hdim],[self.Hdim,self.Hdim]],
            type = "super",
            superrep ="super",
            copy = False
        ...
        if time_sense == 0:
            self.R0_evals,self.R0_evecs = np.linalg.eig(self.R0.full())
        else:
    ...
    def run(self,state00,taulist,*,floquet=False,args=None,e_ops=None,):
        ...
        quasi_e_table = np.exp(np.einsum('i,k -> ki',-1j
                                         * self.floquet_basis.e_quasi,taulist))
        ...
        if results.e_ops:
            for key in results.e_ops.keys():
                try:
                    expects = np.trace(
                        sols_comp_arr @ results.e_ops[key].op(0).full(),
                        axis1=1,axis2=2)
