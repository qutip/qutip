...
Change the constant in power_array to change the starting offresonant laser
    Rabi frequency (in mu-eV)
''' 


power_array = np.linspace(0,60,61, dtype=int)
fsolve_time_array = []
msolve_time_array = []
for idz, E2P in enumerate(power_array):
    print('Working on Spectra number', idz+1, 'of', len(power_array))
    ...
    tlist = np.linspace(0, ttime-dt, Nt)               #Combining everything to make tlist
                                                      ...
                                                      #Taulist Definition
    Ntau =  int((Nt)*100)                                   #50 times the number of points of tlist
    ...
    if idz == 0:
        ...
        ZF = np.zeros( (len(power_array), len(omega_array1)) )
...
# ax.semilogy( (omega_array+(beat/(4*np.pi))), ZF[0], color = 'r' )
...
limits = [omega_array[0]+(beat/(4*np.pi)),\
          omega_array[-1]+(beat/(4*np.pi)),\
          power_array[0] *1000*0.0072992700729927/30 * 4.11,\
          power_array[-1]*1000*0.0072992700729927/30 * 4.11]
...
fig.colorbar(pos)
ax.set_xlabel('$\omega_{beat}-\omega_{emission}$ [THz]')
ax.set_ylabel('$\Omega_{2}$ (GHz)') 
