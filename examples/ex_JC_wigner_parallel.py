...
def func(args):
    ...
    surf=ax.plot_surface(X, Y, W, rstride=1, cstride=1, cmap=cm.jet, alpha=1.0, linewidth=0.05, vmax=0.2, vmin=-0.2)
    ax.set_xlim3d(-5, 5)
    ax.set_ylim3d(-5, 5)
    ...
    #plt.pcolor(X, Y, W)
    #plt.colorbar()
    plt.savefig("jc_animation/jc_model_wigner_"+str(i)+".png")       
    plt.close()
    print i
