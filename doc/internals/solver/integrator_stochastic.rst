...
Stochastic Integrators have the random number generator as part of it's state.
...
The SIntegrator need both drift and diffusion functions, so the supported ``rhs_format`` are different to reflect that:
